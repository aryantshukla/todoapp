...
const taskCompleted = document.querySelector('[data-id="completed"]');
    ...
    return new Promise((resolve) => {
        ...
        function handleClick(event) {
            ...
            if (event.target === submitbtn) {
                ...
                blurContainer.classList.remove('blur');
                modal.removeEventListener('click', handleClick);
                let val = Number((modal.getElementsByTagName('input')[0]).value);
                if (!val)
                    val = -1;
                resolve(val);
            }
...
export function removeTask(_key, event) {
    ...
    askForTime(_key)
        ...
        if (timeTaken > 0) {
            ...
            if (newDiv === null)
                return;
            const divToDeleteData = Database.getItem(_key);
            const iTime = Number(divToDeleteData.timeRequired);
